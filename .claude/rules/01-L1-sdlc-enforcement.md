...
- `directive-protection.sh` (Write/Edit): Blocks Write/Edit to protected directive files (CLAUDE.md, `.claude/rules/`, `docs/architecture/`). Calls `GET /api/v1/agents/by-name/{name}/checks/directive-write` (single API call). **HARD DENY** when no sdlc/maintenance/refactoring story in_progress. Advisory (allow with warning) when qualifying story is active. Fail-closed on API errors
